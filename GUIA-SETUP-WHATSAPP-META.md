...
Configure as seguintes credenciais no arquivo `backend/.env`:

```
META_WHATSAPP_TOKEN=seu_token_aqui
META_WHATSAPP_PHONE_ID=seu_phone_id_aqui
META_WHATSAPP_API_VERSION=v22.0
META_WHATSAPP_FROM_NUMBER=seu_numero_aqui
...
4. Copie o **Temporary Access Token** e **Phone Number ID**
...
Para verificar informações do telefone do usuário:

```bash
python scripts/get_phone_info.py
```
...
Configure seus dados no arquivo `backend/.env`:

- **META_WHATSAPP_TOKEN** - Token de acesso da Meta
- **META_WHATSAPP_PHONE_ID** - ID do número de telefone
- **META_WHATSAPP_FROM_NUMBER** - Número do bot (formato: +15551234567)
